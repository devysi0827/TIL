...
![img](https://blog.kakaocdn.net/dn/bd63H0/btsjYN1NoKS/JxUEKmp4OkaKKCvdImh9NK/img.png)
...
이처럼 applicantion 바깥에 선언할 경우 한 번만 실행하게 됩니다.
...
이 경우, 물품을 두 번 구매하게 됩니다. 이것은, 우리가 Effect에 api를 넣지 말아야할 이유가 됩니다.

우리가 다른 페이지를 갔다가 뒤로 방문하면 또 구매하게됩니다. 이건 우리가 원하는 게 아닙니다. 

구매는 렌더링에 의해서 유발되면 안됩니다. 특별한 상호작용에 의해서 작동해야합니다.

이것은 이펙트에서 제거하고, 구매 버튼의 이벤트 핸들러에 등록합시다.
...
이는 재마운트가 응용프로그램의 논리를 위반하는 경우, 일반적으로 버그가 발견된다는 것을 보여줍니다. 사용자의 입장에서, 뒤로가기를 통한 방문이나 링크를 클릭한 것이나 다르지 않습니다.

React는 다시 마운트하여 구성요소가 이 원칙을 준수하는 지 확인합니다.



### Recap

- 이벤트와 달리, 이펙트는 상호작용이 아니라,렌더링 그 자체에 의해 발생합니다.
- 이펙트는 외부 시스템(API, network)과 동기화할 수 있습니다.
- 기본적으로, 효과는 모든 렌더 후에 실행됩니다.
- React는 모든 종속성 값(dependecy)이 동일할 경우, 해당 Effect를 생략합니다.
- 종속성은 선택할 수 없습니다. Effect 내부 코드에 따라 자동으로 결정됩니다.
- 빈 종속성 배열은 component mount 즉, 화면에 추가되는 것에 해당합니다.
- Strict Mode에서 React는 컴포넌트를 두 번 마운트하여 이펙트를 stress-test합니다.
- 재마운트에 의해서 효과가 중단되면, cleanup function을 구현해야합니다.
- React는 다음 이펙트가 실행되기 전에 그리고 unmount 중에 cleanup function을 호출합니다.
