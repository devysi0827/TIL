...
그런데, 위 예시처럼 x가 여러 개라면 어떤 x가 출력될까? (어떤 x를 참조해야할까?) 이를 자바스크립트 엔진이 결정하는 과정을 **식별자 결정**이라 한다. 그리고 **자바스크립트 엔진이 식별자 결정 시 사용하는 규칙이 스코프**라 할 수 있다.
...
이때, 변수는 자신이 선언되 위치에 의해서 스코프가 결정된다.
...
지역은 함수 몸체 내부를 말하고 지역은 지역 스코프를 만든다. 이는 **코드 블록이 아닌 함수에 의해서 지역스코프가 생성된다는 의미**다.

 

c나 자바 등을 비롯한 대부분의 프로그래밍 언어는 함수 몸체만이 아니라 모든 코드 블록(if, for, while)이 지역 스코프를 만든다. 이런 특성을 **블록 레벨 스코프(block level scope)**라 한다. **하지만 var 키워드로 선언된 변수는 오로지 함수의 코드 블록만을 지역 스코프로 인정**한다. 이러한 특성을 **함수 레벨 스코프(function level scope)**라 한다.
...
\2. 함수를 어디서 정의했는지에 따라 함수의 상위 스코프를 경정한다.
